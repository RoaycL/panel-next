...
`EXT-08` 等具备桌面 Chrome 环境后再人工验收，不阻塞后续开发。`WIDGET-01` 至 `WIDGET-07` 已全部完成；`BRAND-01` 至 `BRAND-06` 与 `OPS-01` 已全部完成，当前转向 `IMAGE-*`：将上传文件管理升级为图库，并按图标/壁纸分类查询（详见 `OPEN_FEATURE_PARITY.md`）。
