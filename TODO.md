...
`EXT-08` 等具备桌面 Chrome 环境后再人工验收，不阻塞后续开发。`WIDGET-01` 至 `WIDGET-07` 已全部完成；`BRAND-01` 至 `BRAND-06` 与 `OPS-01` 已全部完成；已实现外部图床集成和 `IMAGE-01` 至 `IMAGE-05`（图库分类、按分类上传、修改类型、复制链接、统一图库选择器），当前推进 `IMAGE-06`。
