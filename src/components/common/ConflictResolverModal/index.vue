...
<template>
  <NModal
    ...
    :mask-closable="false"
    :closable="false"
    style="max-width: 680px; width: 92vw; border-radius: 20px; overflow: hidden;"
    class="conflict-resolver-modal"
  >
    <NConfigProvider :theme="darkTheme">
      <div class="p-6 bg-slate-950 text-slate-100 space-y-6">
        <!-- 弹窗头部：警告指示 -->
          ...
          <div class="space-y-1 flex-1">
            <div class="flex items-center justify-between">
              <h3 class="text-base font-bold text-white flex items-center space-x-2">
                ...
                <NTag size="small" type="warning" round :bordered="false">需要人工确认</NTag>
              </h3>
            </div>
        ...
        <div class="pt-2 flex flex-col sm:flex-row items-center justify-end space-y-2 sm:space-y-0 sm:space-x-3">
          ...
          >
            <template #icon><SvgIcon icon="material-symbols:content-copy-outline" /></template>
            另存为离线副本 (保留两份)
          </NButton>
          ...
          >
            <template #icon><SvgIcon icon="material-symbols:cloud-download-outline" /></template>
            保留云端版本 (丢弃离线修改)
          </NButton>
          ...
          >
            <template #icon><SvgIcon icon="material-symbols:cloud-upload-outline" /></template>
            保留本地修改 (覆盖云端)
          </NButton>
        </div>
      </div>
...
<style scoped>
.conflict-resolver-modal :deep(.n-card__content) {
  padding: 0 !important;
}
</style>
