...
- [x] `CARD-04` 分组样式独立配置、公开模式隐藏、唯一标识及稳定 CSS class。实现：分组容器增加 `group-{id}` 稳定 CSS class，可通过自定义 CSS 独立配置样式。代码：`src/views/home/index.vue`。
- [x] `CARD-05` 卡片数据本地缓存、加载指示和受控缓存空间分配。实现：getList 先读取 persistentStorage 缓存快速渲染，后台刷新后更新缓存。代码：`src/views/home/index.vue`。
...
- [x] `CARD-08` 实验性智能选择内网/默认地址。实现：smartSelectItemUrl 并行探测 wan/lan 地址延迟，选更快的；结果缓存在内存 Map。代码：`src/dashboard/core.ts`。
