...
- [ ] `DATA-03` 覆盖所有账号的配置、图标和壁纸；对外部 MySQL 明确提示并提供本 Fork 的可移植逻辑导出方案。
...
1. `DATA-*`：备份、恢复、迁移与安全增强。
2. `BRAND-*` + `OPS-01`：全局设置和配置目录迁移。
3. `IMAGE-*`：图库、分类和公共图库。
4. `AUTH-*`：验证码、多账号切换和窗口系统。
5. `DOCKER-*` + Docker 相关 `CARD-*`：Docker 管理和权限。
6. 其余 `CARD-*`、`SEARCH-*`：导航交互、缓存和搜索。
7. `API-*` + 其余 `OPS-*`：OpenAPI、HTTPS、静态目录和诊断。
